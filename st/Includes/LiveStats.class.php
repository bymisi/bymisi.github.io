...
class LiveStats {
	...
	public function __construct($hostname, $port = 27015) {
		...
		$errno = 0; $errstr = '';
		$this->Sock = @stream_socket_client("udp://{$this->IP}:{$this->Port}", $errno, $errstr, 10);
		...
		if (stream_set_timeout($this->Sock, 10) == false)
	...
	function GetServer() {
		...
		while (!($this->HaveRules && $this->HaveInfo && $this->HavePlayer)) {
			...
			$packet = stream_socket_recvfrom($this->Sock, 2048);
